...
func (s *Server) ProjectsTable(w http.ResponseWriter, r *http.Request) {
	...
	if r.Form.Get("gross_area") != "" {
		...
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte("El área bruta debe ser un número válido"))
			return
		}
	}
	if r.Form.Get("net_area") != "" {
		...
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte("El área neta debe ser un número válido"))
			return
		}
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "duplicate") {
			...
			_, _ = w.Write([]byte(fmt.Sprintf("El nombre %s ya existe", p.Name)))
			return
		}
...
func (s *Server) ProyectDisplay(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		_, _ = w.Write([]byte("Proyecto no encontrado"))
		return
	}
