...
func (s *Server) DetailsTable(w http.ResponseWriter, r *http.Request) {
	...
	if biId == "" {
		...
		_, _ = w.Write([]byte("Ingrese una partida"))
		return
	}
	parsedBudgetItemId, err := uuid.Parse(biId)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte("Ingrese una partida válida"))
		return
	}
	...
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte("Cantidad debe ser un número válido"))
		return
	}
	...
	if c == "" {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte("Ingrese un costo"))
		return
	}
	cost, err := strconv.ParseFloat(c, 64)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte("Costo debe ser un número válido"))
		return
	}
	...
	if err := s.DB.AddDetail(d); err != nil {
		if strings.Contains(err.Error(), "duplicate") {
			...
			_, _ = w.Write([]byte("Ya existe una partida con ese nombre en la factura"))
			return
		}
		if strings.Contains(err.Error(), "no rows") {
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte("No existe presupuesto para esa partida"))
			return
		}
		w.WriteHeader(http.StatusInternalServerError)
		_, _ = w.Write([]byte(fmt.Sprintf("Error al insertar partida. Err: %s", err.Error())))
...
func (s *Server) DetailsTableDisplay(w http.ResponseWriter, r *http.Request) {
	ctx, _ := utils.GetMyPaload(r)
	id := chi.URLParam(r, "invoiceId")
	parsedInvoiceId, _ := uuid.Parse(id)
